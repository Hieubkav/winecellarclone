...
# Backend/Frontend Control Rules
- Scope ownership:
  - Backend: API contract, schema/migration, cache, watermark processing.
  - Frontend: route/UI/state/UX, admin settings form, typings dùng cho UI.
- Sync checklist khi đổi API/settings:
  1) Backend: update validation + response fields + defaults.
  2) Frontend: update types + load/save payload + UI fallback.
  3) Cache/ETag: update hash nếu field ảnh hưởng output.
- Commit tách repo bắt buộc:
  - Backend repo: commit message dạng `backend: ...`.
  - Frontend repo: commit message dạng `frontend: ...`.
  - Không trộn thay đổi 2 repo trong 1 commit.
