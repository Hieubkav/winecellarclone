...
export default async function CollectionChildPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const copy = buildStaticChildCopy(HUB, slug);
  return <SeoSimplePage eyebrow={copy.eyebrow} title={copy.heading} description={copy.description} parentHref={`/${HUB}`} parentLabel="Bộ sưu tập" />;
}
